...
def get_location_choices(df: pd.DataFrame) -> List[str]:
    ...
    """
    if 'department_code' in df.columns and 'department_name' in df.columns:
        departments = df[['department_code', 'department_name']].drop_duplicates()
        return sorted([f"{row['department_code']} - {row['department_name']}" 
                      for _, row in departments.iterrows()])
    elif 'department_code' in df.columns:
        departments = df['department_code'].unique()
        return sorted([f"{dept}" for dept in departments if pd.notna(dept)])
    return []
...
def filter_data_by_location(df: pd.DataFrame, location: str) -> pd.DataFrame:
    ...
    try:
        ...
        return df[df['department_code'] == int(location_code)]
...
def create_sales_by_property_type_plot(df: pd.DataFrame) -> go.Figure:
    ...
    if df.empty:
        fig = go.Figure()
        fig.update_layout(title="Sales by property type",
...
def create_volume_evolution_plot(df: pd.DataFrame) -> go.Figure:
    ...
    if df.empty or 'transaction_date' not in df.columns:
        fig = go.Figure()
        fig.update_layout(title="Sales volume evolution",
                         xaxis_title="Date",
                         yaxis_title="Number of sales")
        return fig
...
def get_sales_by_type_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if df.empty or 'property_type' not in df.columns:
        return pd.DataFrame({
            'property_type': ['No data available'],
            'transaction_count': [0]
        })
...
def calculate_financing_simulation(property_price: float, 
                                 personal_contribution: float, 
                                 loan_duration: int, 
                                 interest_rate: float) -> Tuple[float, float, float]:
    """
...
def compare_departments(df: pd.DataFrame, selected_departments: List[str]) -> pd.DataFrame:
    ...
    """
    from .market_analysis import filter_data_by_location, calculate_market_metrics
