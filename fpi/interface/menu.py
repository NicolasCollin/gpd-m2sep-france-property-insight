...
def app_menu() -> gr.Blocks:
    ...
    with gr.Blocks(css=global_css, title="France Property Insight", fill_width=True) as menu:
        ...
        with gr.Column(visible=True, elem_classes="page-content") as home:
            ...
            department_dropdown, search_button, dashboard_card, estimation_card, about_card = home_page()
        ...
        nav_api_docs.click(
            lambda: gr.HTML(
                "<script>window.open('https://france-property-insight-docs.onrender.com/fpi.html', '_blank')</script>"
            )
        )
        nav_gitlab.click(
            lambda: gr.HTML(
                "<script>window.open('https://gitlab-mi.univ-reims.fr/phan0005/gpd-m2sep-france-property-insight', '_blank')</script>"
            )
        )
