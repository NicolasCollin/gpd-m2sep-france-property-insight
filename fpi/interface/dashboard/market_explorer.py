...
def get_market_explorer_tab(df: pd.DataFrame) -> None:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            property_price = gr.Number(
                label="Property price (€)", 
                interactive=True,
                minimum=0
            )
            personal_contribution = gr.Number(
                label="Personal contribution (€)", 
                interactive=True,
                minimum=0
            )
            loan_duration = gr.Slider(
    ...
    def update_market_analysis(location: str) -> Dict[str, Any]:
        """
        ...
        return {
            avg_price_m2: metrics['avg_price_per_m2'],
    ...
    location_dropdown.change(
        ...
        outputs=[
            avg_price_m2, total_transactions, median_price,
            sales_by_type_plot, volume_evolution_plot
        ]
    )
    ...
    if get_location_choices(df):
        initial_update = update_market_analysis(get_location_choices(df)[0])