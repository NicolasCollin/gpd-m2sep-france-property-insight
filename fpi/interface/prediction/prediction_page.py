...
def api_run_prediction(postal: str, prop_type: str, area: float, rooms: int, land: float) -> str:
    """
...
def run_prediction(postal: str, prop_type: str, area: float, rooms: int, land: float) -> str:
    """
    Compute a prediction using the model locally instead of through the API.

    Parameters:
        postal (str): Postal code.
        prop_type (str): Property type ("House", "Apartment").
        area (float): Living area in square meters.
        rooms (int): Number of rooms.
        land (float): Land area in square meters.

    Returns:
        str: A formatted string with the estimated price or an error description.
    """
    error_msg: str | None = validate_inputs(postal, prop_type, area, rooms, land)
    if error_msg:
        return error_msg
